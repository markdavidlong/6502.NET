...
public class MiscOperations : CPUTest {
    ...
    public void NOP() {
        ...
        uint8_t[] originalMemory = this.m_Bus.RAM.DataArray(); //Data.ToArray();
        ExecuteInstruction(0xEA, m_CPU); // Opcode for NOP
        ExecuteInstruction(0x1C, m_CPU); // Another NOP opcode
        uint8_t[] newMemory = this.m_Bus.RAM.DataArray(); //Data.ToArray();
    ...
    public void XXX() {
        ...
        uint8_t[] originalMemory = this.m_Bus.RAM.DataArray(); //.ToArray();
        ExecuteInstruction(0x02, m_CPU); // One of the illegal opcodes
        uint8_t[] newMemory = this.m_Bus.RAM.DataArray(); //.ToArray();
